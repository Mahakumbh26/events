...
# --------------------------------------------------------------------
# Harvest status overrides per plot, loaded from JSON
#   - generated locally from `UP Pilot Harvesting Logic..xlsx`
#   - stored in `harvest_overrides.json` alongside this file
#   - structure: { "<plot_name>": "<Status>", ... }
# --------------------------------------------------------------------
HARVEST_STATUS_OVERRIDES: Dict[str, str] = {}
try:
    _json_path = os.path.join(os.path.dirname(__file__), "harvest_overrides.json")
    if os.path.exists(_json_path):
        with open(_json_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        if isinstance(data, dict):
            HARVEST_STATUS_OVERRIDES = {
                str(k).strip(): str(v).strip()
                for k, v in data.items()
                if v is not None
            }
            print(f"Loaded {len(HARVEST_STATUS_OVERRIDES)} harvest overrides from JSON")
        else:
            print("Warning: harvest_overrides.json must contain a JSON object {plot_name: status}")
    else:
        print("harvest_overrides.json not found; no manual harvest overrides loaded")
except Exception as _e:
    print(f"Warning: failed to load harvest_overrides.json: {_e}")
...
def get_grapes_brix_image(start_date: str, end_date: str, geometry):
    s2 = (
    ...
    ph = brix.multiply(0.0697961).add(1.665897).rename("pH")
    ta = brix.multiply(-1.0461971).add(28.710605).rename("TA")
 
    return brix.addBands(ph).addBands(ta)
 
def compute_grapes_quality_stats(image, geometry, plot_id: str):
    fc = ee.FeatureCollection([ee.Feature(geometry, {"plot": plot_id})])
    ...
    stats = image.reduceRegions(fc, reducer, 10).getInfo()
    props = stats["features"][0]["properties"]
    ...
    return {
        "brix": {
            "mean": safe(props.get("Brix_mean")),
            "min": safe(props.get("Brix_min")),
            "max": safe(props.get("Brix_max")),
            "median": safe(props.get("Brix_median")),
            "stdDev": safe(props.get("Brix_stdDev")),
        },
        "ph": {
            "mean": safe(props.get("pH_mean")),
            "min": safe(props.get("pH_min")),
            "max": safe(props.get("pH_max")),
            "median": safe(props.get("pH_median")),
            "stdDev": safe(props.get("pH_stdDev")),
        },
        "ta": {
            "mean": safe(props.get("TA_mean")),
            "min": safe(props.get("TA_min")),
            "max": safe(props.get("TA_max")),
            "median": safe(props.get("TA_median")),
            "stdDev": safe(props.get("TA_stdDev")),
        },
    }
...
# Cache for agroStats endpoint only (4 hours)
agrostats_cache = TTLCache(maxsize=2000, ttl=86400)
...
def get_all_stats(
    ...
    # --------------------------------------------------
    # 🔁 In-memory cache (SAFE with response_model)
    # --------------------------------------------------
    ...
    # If Django is down → plot_dict is empty → return empty dict safely
    if not plot_dict:
...
async def sugarcane_harvest_endpoint(
    ...
    # If plot has override status (e.g. from Excel), return it without requiring plantation_date or running EE
    override_status = HARVEST_STATUS_OVERRIDES.get(plot_name)
    if override_status is not None:
        feature = {
            ...
            "properties": {
                ...
                "harvest_date": None,
                "has_harvest": None,
                "harvest_status": override_status,
                "image_count": 0,
                "last_updated": datetime.now().isoformat(),
                "source": "harvest_overrides",
            },
        }
        return {
    ...
    if low is not None:
        if month_diff < low:
            ...
            status = "Harvested" if result["has_harvest"] else "Ready to harvest"
        else:
...
async def refresh_from_django():
    """Manually refresh all plots from Django and clear agroStats cache"""
    try:
        global plot_dict
        print("ðŸ”„ Manual refresh from Django requested for events.py...")
        plot_dict = plot_sync_service.get_plots_dict(force_refresh=True)
        agrostats_cache.clear()
        return {"status": "success", "message": f"Refreshed {len(plot_dict)} plots and cleared agroStats cache."}
    except Exception as e:

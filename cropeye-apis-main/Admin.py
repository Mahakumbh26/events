...
import os, json
from events import get_grapes_brix_image,compute_grapes_quality_stats
...
def detect_pests_by_plot(
    ...
    if "grape" in crop_type:
        # ---- ORIGINAL GRAPE LOGIC (UNCHANGED) ----
        analysis_start = ee.Date(start_date)
        analysis_end = ee.Date(end_date)
        ...
        result = {
            ...
            "image_count": image_count,
            "masks": {
                "chewing": chewing_mask,
                "fungi": fungi_mask,
                "sucking": sucking_mask,
                "soilborne": soilborne_mask,
            },
        }
    ...
    # ========================================================
        ...
        # ---- ORIGINAL SUGARCANE LOGIC (UNCHANGED) ----
        analysis_start = ee.Date(start_date)
        analysis_end = ee.Date(end_date)
        ...
        pest_order = ["chewing", "fungi", "sucking", "wilt", "soilborne"]
        class_map = {
            "chewing": 1,
            "fungi": 2,
            "sucking": 3,
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Pest detection not supported for crop type: {crop_type}",
        )
 
    return result, pest_order, class_map, plot_data, crop_type
...
@app.post("/pest-detection-by-crop", response_model=Dict[str, Any])
async def pest_detection_by_crop(
    plot_name: str,
    start_date: str | None = Query(None, description="YYYY-MM-DD (optional)"),
    end_date: str | None = Query(None, description="YYYY-MM-DD (optional)"),
):
    # -------------------- DEFAULT DATES --------------------
    ...
    for pest in pest_order:
        combined = combined.where(result["masks"][pest], class_map[pest])
 
    combined = combined.clip(geometry).focal_mean(10, units="meters")
    ...
    pixel_counts = {p: count(result["masks"][p]) for p in pest_order}
 
    total_pixels = int(
        one.reduceRegion(ee.Reducer.count(), geometry, 10, bestEffort=True)
        .get("constant")
        .getInfo()
    )
    ...
    def coords(mask):
        pts = (
            mask.selfMask()
            .addBands(ee.Image.pixelLonLat())
            .sample(geometry, 10, geometries=True, tileScale=4)
            .getInfo()
        )
        return list(
    ...
    pixel_coords = {p: coords(result["masks"][p]) for p in pest_order}
    ...
    return {
        ...
        "pixel_summary": {
            ...
            **{
            ...
            "baseline_start_date": result["baseline_start"].format("YYYY-MM-dd").getInfo(),
            "baseline_end_date": result["baseline_end"].format("YYYY-MM-dd").getInfo(),
            "analysis_start_date": result["analysis_start"].format("YYYY-MM-dd").getInfo(),
            "analysis_end_date": result["analysis_end"].format("YYYY-MM-dd").getInfo(),
        },
    }
 
@app.post("/grapes/canopy-vigour", response_model=Dict[str, Any])
def grapes_canopy_vigour(
    ...
    # ------------------------------------------------------------------
    # Geometry (ENSURE EE GEOMETRY)
    # ------------------------------------------------------------------
    geometry = ee.Geometry(plot_dict[plot_name]["geometry"])
    ...
    tile_url = vigour_class_img.getMapId(vigour_vis)["tile_fetcher"].url_format
    ...
    return {
        ...
        "vigour_class": (
            "Poor" if ndvi_mean < 0.3 else
            "Moderate" if ndvi_mean < 0.5 else
            "Good" if ndvi_mean < 0.7 else
            "Excellent"
        ) if ndvi_mean is not None else None,
        "last_updated": datetime.utcnow().isoformat(),
    }
 

 
#####----------------End Of Grapes Endpoints---------------------#####    
@app.post("/grapes-brix-gridmap", response_model=Dict[str, Any])
def grapes_quality_stats(
    plot_name: str = Query(...),
    ...
    if not plot:
        raise HTTPException(status_code=404, detail="Invalid plot_name")
 
    geometry = plot["geometry"]
 
    # -------------------- INLINE CROP TYPE (CONSISTENT LOGIC) --------------------
    ...
    # -------------------- EXISTING GRAPES LOGIC (UNCHANGED) --------------------
    image = get_grapes_brix_image(start_date, end_date, geometry)
    stats = compute_grapes_quality_stats(image, geometry, plot_name)
    ...
    feature = {
        ...
        "properties": {
            "plot_name": plot_name,
            "crop": crop_type,  # ✅ FIXED
            "date": end_date,
            "brix_statistics": stats["brix"],
            "recovery_statistics": stats["ph"],
            "sugar_yield_statistics": stats["ta"],
